...
public class CartAPIHelper
{
    ...
    {
        var response = await _httpClient.PostAsJsonAsync("https://jollicowbe-admin.up.railway.app/api/admin/carts/create", cart);
    ...
    public static async Task<decimal> GetCartTotalAsync(string idTable, string restaurantId)
    {
        try
        {
            ...
            var response = await _httpClient.PostAsJsonAsync("https://jollicowbe-admin.up.railway.app/api/admin/carts/filter", payload);
