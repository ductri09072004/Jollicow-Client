...
public class CartAPIHelper
{
    ...
    {
        var response = await _httpClient.PostAsJsonAsync("https://jollicowbe-admin.up.railway.app/api/carts/create", cart);
    ...
    public static async Task<decimal> GetCartTotalAsync(string idTable, string restaurantId)
    {
        try
        {
            ...
            var response = await _httpClient.PostAsJsonAsync("https://jollicowbe-admin.up.railway.app/api/carts/filter", payload);
